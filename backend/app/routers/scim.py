...
async def get_user(user_id: uuid.UUID, db: AsyncSession = Depends(get_db)) -> ScimUserResource:
...
async def create_user(body: ScimCreateUser, db: AsyncSession = Depends(get_db)) -> ScimUserResource:
    email = body.userName
    if body.emails:
