...
from app.history_display import _friendly_hardware_value_dict, _hardware_assignee_label


class HardwareAssetFriendlyAuditDictTest(unittest.TestCase):
    def test_dedupes_hardware_status_when_status_present(self) -> None:
        ...
        out = _friendly_hardware_value_dict(d)
    ...
    def test_renames_hardware_status_when_status_absent(self) -> None:
        out = _friendly_hardware_value_dict({"hardware_status_id": "Assigned"})
        self.assertEqual(out["status"], "Assigned")
        self.assertNotIn("hardware_status_id", out)

    def test_labels_operating_system(self) -> None:
        out = _friendly_hardware_value_dict({"operating_system": "macos"})
        self.assertEqual(out["operating_system"], "macOS")

    def test_labels_mdm_connected(self) -> None:
        self.assertEqual(
            _friendly_hardware_value_dict({"mdm_connected": True})["mdm_connected"],
            "Yes",
        )
        self.assertEqual(
            _friendly_hardware_value_dict({"mdm_connected": False})["mdm_connected"],
            "No",
        )


class HardwareAssetAssigneeLabelTest(unittest.TestCase):
    def test_prefers_first_last(self) -> None:
        ...
        self.assertEqual(_hardware_assignee_label(u), "Dwight Schrute")
    ...
    def test_falls_back_to_email(self) -> None:
        ...
        self.assertEqual(_hardware_assignee_label(u), "only@b.com")
