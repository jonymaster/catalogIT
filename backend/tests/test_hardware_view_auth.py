...
from app.dependencies.auth import ensure_hardware_view_access


class EnsureHardwareViewAccessTest(unittest.TestCase):
    ...
    def test_viewer_without_permission_row_403(self) -> None:
        async def run() -> None:
            user = MagicMock()
            user.id = uuid.uuid4()
            user.role = "viewer"
