...
from app.routers.hardware_assets import search_hardware_assets
...
class HardwareAssetSearchTest(unittest.IsolatedAsyncioTestCase):
    async def test_blank_query_returns_no_rows_without_database_query(self) -> None:
        db = _FakeDb()

        result = await search_hardware_assets(q="   ", _hw=SimpleNamespace(), db=db)

        self.assertEqual(result, [])
        self.assertEqual(db.statements, [])

    async def test_search_query_matches_serial_number(self) -> None:
        hardware = SimpleNamespace(serial_number="C02FABRIC01")
        db = _FakeDb(rows=[hardware])

        result = await search_hardware_assets(
            q="C02FABRIC01",
            limit=20,
            _hw=SimpleNamespace(),
            db=db,
        ...
        self.assertIn("hardware_assets.serial_number", compiled)
